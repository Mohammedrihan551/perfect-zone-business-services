...
export default function OffshoreCompanyPopup({
  open,
  onClose,
  currentStep,
  name,
  ...
  const t = isArabic
    ? {
        ...
        companyStatus: "شركة جديدة أم قائمة؟",
        offshoreJurisdiction: "الولاية القضائية المفضلة",
        activity: "النشاط التجاري بالتفصيل *",
        shareholders: "عدد المساهمين *",

        internationalBusiness: "هل نشاطك دولي؟",
        bankAccount: "هل تحتاج إلى حساب مصرفي؟",
        nomineeService: "هل تحتاج إلى خدمة Nominee؟",
        officeRequired: "هل تحتاج إلى مكتب افتراضي؟",

        assetProtection: "حماية الأصول؟",
        holdingCompany: "شركة قابضة؟",
        taxOptimization: "تحسين ضريبي؟",

        timeline: "الجدول الزمني",
        budget: "الميزانية",
        accounting: "خدمات المحاسبة؟",
        trademark: "تسجيل العلامة التجارية؟",
    ...
    : {
        ...
        setupTag: "Offshore Company Setup",
        ...
        companyStatus: "New or Existing Business?",
        offshoreJurisdiction: "Preferred Jurisdiction",
        activity: "Detailed Business Activity *",
        shareholders: "Number of Shareholders *",

        internationalBusiness: "International Business Activity?",
        bankAccount: "Need Bank Account Assistance?",
        nomineeService: "Need Nominee Service?",
        officeRequired: "Need Virtual Office?",

        assetProtection: "Asset Protection?",
        holdingCompany: "Holding Company Structure?",
        taxOptimization: "Tax Optimization?",

        timeline: "Setup Timeline",
        budget: "Budget",
        accounting: "Need Accounting Services?",
  ...
  const validateStep = () => {
    ...
    if (step === 2) {
      ...
      if (!form.owners)
        newErrors.owners = "Please select number of shareholders";

      if (!form.activity.trim())
        newErrors.activity = "Business Activity is required";
    }

    if (step === 3) {
      if (!form.bankAccount)
        newErrors.bankAccount = "Please select bank account assistance";

      if (!form.officeRequired)
        newErrors.officeRequired = "Please select office requirement";
    }

    if (step === 4) {
      if (!form.timeline) newErrors.timeline = "Please select setup timeline";
      if (!form.budget) newErrors.budget = "Please select your budget";
    }
  ...
  const handleSubmitEmail = () => {
    ...
    const subject = "OFFSHORE COMPANY SETUP Inquiry";
...
Jurisdiction: ${form.offshoreJurisdiction}
Number of Owners: ${form.owners}

International Business: ${form.internationalBusiness}
Bank Account Assistance: ${form.bankAccount}
Nominee Service: ${form.nomineeService}
Virtual Office: ${form.officeRequired}

Asset Protection: ${form.assetProtection}
Holding Structure: ${form.holdingCompany}
Tax Optimization: ${form.taxOptimization}
...
`;
  ...
  const handleSubmitWhatsApp = () => {
    if (!validateStep()) return;
    const message = `
*OFFSHORE COMPANY SETUP Inquiry*
        ...
        <div className="p-6">
          ...
          {step === 2 && (
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <Select
                label={t.offshoreJurisdiction}
                name="offshoreJurisdiction"
                value={form.offshoreJurisdiction}
                onChange={handleChange}
                options={[
                  "JAFZA Offshore",
                  "RAK ICC",
                ...
                error={errors.offshoreJurisdiction}
              />
          ...
          {step === 3 && (
            <div className="grid gap-5 md:grid-cols-2">
              <Select
                label={t.internationalBusiness}
                name="internationalBusiness"
                value={form.internationalBusiness}
                onChange={handleChange}
                options={["Yes", "No"]}
                error={errors.internationalBusiness}
              />
              ...
              <Select
                label={t.nomineeService}
                name="nomineeService"
                value={form.nomineeService}
                onChange={handleChange}
                options={["Yes", "No"]}
                error={errors.nomineeService}
              />

              <Select
                label={t.officeRequired}
                name="officeRequired"
                value={form.officeRequired}
                onChange={handleChange}
                options={["Yes", "No"]}
                error={errors.officeRequired}
              />

              <Select
                label={t.assetProtection}
                name="assetProtection"
                value={form.assetProtection}
                onChange={handleChange}
                options={["Yes", "No"]}
                error={errors.assetProtection}
              />

              <Select
                label={t.holdingCompany}
                name="holdingCompany"
                value={form.holdingCompany}
                onChange={handleChange}
                options={["Yes", "No"]}
                error={errors.holdingCompany}
              />

              <Select
                label={t.taxOptimization}
                name="taxOptimization"
                value={form.taxOptimization}
                onChange={handleChange}
                options={["Yes", "No"]}
                error={errors.taxOptimization}
              />
            </div>
          )}
