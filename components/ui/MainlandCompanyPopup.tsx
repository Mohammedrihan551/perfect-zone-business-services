...
export default function MainlandCompanyPopup({
  open,
  onClose,
  currentStep,
  name,
  ...
  const t = isArabic
    ? {
        ...
        setupTag: "تأسيس شركة في البر الرئيسي",
        ...
        revenue: "الإيرادات السنوية المتوقعة",
        importExport: "استيراد / تصدير؟",
        localMarket: "البيع داخل السوق الإماراتي؟",
        ecommerce: "أنشطة التجارة الإلكترونية؟",
        governmentContracts: "العقود الحكومية؟",
        shareholders: "عدد المساهمين *",
        activity: "النشاط التجاري بالتفصيل *",

        visaRequired: "هل تحتاج إلى إقامة في الإمارات؟",
        visaCount: "عدد التأشيرات",
        familyVisa: "هل تحتاج إلى كفالة عائلية؟",
        investorVisa: "هل تحتاج إلى تأشيرة مستثمر؟",

        officeRequired: "هل تحتاج إلى مكتب؟",
        emirate: "الإمارة المفضلة",
        banking: "هل تحتاج إلى مساعدة مصرفية؟",
    ...
    : {
        ...
        localMarket: "Sell in UAE Market?",
        ecommerce: "E-Commerce Activities?",
        governmentContracts: "Government Contracts?",
        shareholders: "Number of Shareholders *",
        activity: "Detailed Business Activity *",

        visaRequired: "Need UAE Residence Visa?",
        visaCount: "Number of Visas *",
        familyVisa: "Family Sponsorship Needed?",
        investorVisa: "Investor Visa Needed?",

        officeRequired: "Office Required? *",
        emirate: "Preferred Emirate *",
        banking: "Need Bank Account Assistance? *",
  ...
  const validateStep = () => {
    ...
    if (step === 3) {
      if (!form.emirate) newErrors.emirate = "Please select preferred emirate";

      if (!form.banking) newErrors.banking = "Please select banking assistance";

      if (!form.officeRequired)
        newErrors.officeRequired = "Please select office requirement";

      if (form.visaRequired === "Yes") {
        if (!form.visaCount)
          newErrors.visaCount = "Please select visa quantity";
      }
    }
  ...
  const handleSubmitEmail = () => {
    ...
    const subject = "Mainland Company Formation Inquiry";
...
Visa Required: ${form.visaRequired}
Number of Visas: ${form.visaCount}

Office Required: ${form.officeRequired}

Preferred Emirate: ${form.emirate}

Bank Account Assistance: ${form.banking}
...
`;
  ...
  const handleSubmitWhatsApp = () => {
    if (!validateStep()) return;
    const message = `
*Mainland Company Formation Inquiry*
        ...
        <div className="p-6">
          ...
          {step === 2 && (
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <Select
                label={t.governmentContracts}
                name="governmentContracts"
                value={form.governmentContracts}
                onChange={handleChange}
                options={["Yes", "No"]}
                error={errors.governmentContracts}
              />
              ...
              {form.visaRequired === "Yes" && (
                <>
                  <Select
                    label={t.visaCount}
                    name="visaCount"
                    value={form.visaCount}
                    onChange={handleChange}
                    options={["1", "2-5", "5-10", "10+"]}
                    error={errors.visaCount}
                  />

                  <Select
                    label={t.familyVisa}
                    name="familySponsorship"
                    value={form.familySponsorship}
                    onChange={handleChange}
                    options={["Yes", "No"]}
                    error={errors.familySponsorship}
                  />

                  <Select
                    label={t.investorVisa}
                    name="investorVisa"
                    value={form.investorVisa}
                    onChange={handleChange}
                    options={["Yes", "No"]}
                    error={errors.investorVisa}
                  />
                </>
              )}
          ...
          {step === 4 && (
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <Select
                ...
                value={form.vatRegistration}
                onChange={handleChange}
                options={["Yes", "No"]}
                error={errors.vatRegistration}
              />
