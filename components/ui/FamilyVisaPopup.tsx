...
export default function FamilyVisaPopup({
  open,
  onClose,
  currentStep,
  name,
  ...
  const t = isArabic
    ? {
        ...
        visaType: "نوع التأشيرة العائلية *",
        sponsorStatus: "حالة الكفيل *",
        residenceStatus: "حالة الإقامة الحالية *",
        dependentsCount: "عدد أفراد الأسرة *",

        relationshipType: "صلة القرابة",
        incomeRange: "الدخل الشهري",
        documentsReady: "هل المستندات جاهزة؟",
        medicalRequired: "فحص طبي مطلوب؟",
    ...
    : {
        ...
        setupTag: "Family Visa Assistance",
        ...
        visaType: "Family Visa Type *",
        sponsorStatus: "Sponsor Status *",
        residenceStatus: "Current Residence Status *",
  ...
  const validateStep = () => {
    ...
    if (step === 2) {
      ...
      if (!form.sponsorStatus)
        newErrors.sponsorStatus = "Please select sponsor status";

      if (!form.residenceStatus)
        newErrors.residenceStatus = "Please select residence status";

      if (!form.dependentsCount)
        newErrors.dependentsCount = "Please select number of dependents";
    }
  ...
  const handleSubmitEmail = () => {
    ...
    const subject = "Family Visa Inquiry";
...
Sponsor Status: ${form.sponsorStatus}
Residence Status: ${form.residenceStatus}
Dependents Count: ${form.dependentsCount}

Relationship Type: ${form.relationshipType}
Income Range: ${form.incomeRange}
          ...
          {step === 3 && (
            <div className="grid gap-5 md:grid-cols-2">
              <Select
                ...
                onChange={handleChange}
                options={["Spouse", "Child", "Parent", "Mixed Family"]}
